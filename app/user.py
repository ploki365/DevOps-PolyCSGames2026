...
def create_user(
    payload: schemas.UserBaseSchema, db: Session = Depends(get_db)  # noqa: B008
):
    try:
...
def get_user(userId: str, db: Session = Depends(get_db)):  # noqa: B008
...
def delete_user(userId: str, db: Session = Depends(get_db)):  # noqa: B008
    try:
        user_query = db.query(models.User).filter(models.User.id == userId)
